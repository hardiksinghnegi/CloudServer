(function($) {
    ...
    $.fn.orgChart.defaults = {
        data: [{id:1, name:'Name', designation:'designation', dept:'department', photo:'<img src="" width="50" height="50">', parent: 0}],
        showControls: false,
        allowEdit: false,
        onAddNode: null,
        onDeleteNode: null,
    ...
    function Node(data){
        ...
        this.render = function(opts){
            ...
            mainTable = "<table cellpadding='0' cellspacing='0' border='1'>";
            var nodeColspan = childLength>0?2*childLength:2;
            mainTable += "<tr><td colspan='"+nodeColspan+"'>"+self.formatNode(opts)+"</td></tr>";

            if(childLength > 0){
                var downLineTable = "<table cellpadding='0' cellspacing='0' border='1'><tr class='lines x'><td class='line left half'></td><td class='line right half'></td></table>";
        ...
        this.formatNode = function(opts){
            var nameString = '',
                ...
                photoString = '',
                designationString = '';
            view = "<button class='viewButtonModal' type='image' src='../assets/img/edit.png' onclick='viewButtonHandler(" + this.data.id + ")' id='viewButton" + self.data.id + "'>View</button>";
           
            if(typeof data.name !== 'undefined'){
                nameString = '<h2 >'+self.data.name+'</h2>';
            }
            if(typeof data.dept !== 'undefined'){
                emailString = '<h6 class="peopleEmail">'+self.data.dept+'</h6>';
            }
            if(typeof data.designation !== 'undefined'){
                designationString = '<p class="peopleDesignation margin-zero">'+self.data.designation+'</p>';
            }
            if(typeof data.dept !== 'undefined'){
                departmentString = '<p class="peopleDepartment">'+self.data.dept+'</p>';
            }
            if(typeof data.photo !== 'undefined'){
                photoString = '<img src="'+self.data.photo+'" class="" width="50" height="50">';
            }
			...
			if(this.data.rm.toLowerCase() == "none")
			{
				
				return "<div class='node wow zoomIn' data-wow-duration='0.5s' node-id='"+this.data.id+"'><div class='editCont'><form action='../hierarchy/editHierarchy' method='post'><input type='image' src='../assets/img/pencil.png' ><input type='hidden' name='editid' id ='editid' value='"+this.data.email+"' ></form></div>"+
				"<div class=row><div class='col-sm-4'>"+photoString+"</div><div class='col-sm-8'><div class='slideInDiv'>"+designationString+departmentString+"</div>"+nameString+emailString+buttonsHtml+"</div></div></div>";
			}
			else
			{
				return "<div class='node wow zoomIn' data-wow-duration='0.5s' node-id='"+this.data.id+"'><div class='editCont'><form action='../hierarchy/editHierarchy' method='post'><input type='image' src='../assets/img/pencil.png' ><input type='hidden' name='editid' id ='editid' value='"+this.data.email+"' ></form></div>"+
				"<div class='deleteCont'><form action='../hierarchy/deleteHierarchy' method='post'><input type='image' src='../assets/img/time.png' ><input type='hidden' name='deleteid' id ='deleteid' value='"+this.data.email+"' ></form></div><div class=row><div class='col-sm-4'>"+photoString+"</div><div class='col-sm-8'><div class='slideInDiv'>"+designationString+departmentString+"</div>"+nameString+emailString+buttonsHtml+"</div></div></div>";
			}
		}
    }
